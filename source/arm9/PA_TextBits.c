...
void PA_DoAlign(u16 start, s16 x, s16 maxx, u8 justify) {
	...
	if (textinfo.align == ALIGN_RIGHT) { // Cas simple, on rajoute la diff�rence de largeur � toutes les lettres...
		width++;

		for (i = start; i < PA_LetterPos.NLetters; i++) PA_LetterPos.Letter[i].X += width;
	} else if (textinfo.align == ALIGN_CENTER) { // Cas simple, on rajoute la moiti� de la largeur � toutes les lettres...
		width = (width + 1) >> 1;

		for (i = start; i < PA_LetterPos.NLetters; i++) PA_LetterPos.Letter[i].X += width;
	} else if (justify && (textinfo.align == ALIGN_JUSTIFY)) { // Cas relou ^^
		//u8 nletters = PA_LetterPos.NLetters-start; // Nombre de lettres
		u8 justify = 0; // D�callage � faire au d�but...
		u8 spaces = 0;
		u8 change = 0;
		u8 quickadd = 0;
		...
		for (i = start; i < PA_LetterPos.NLetters; i++) {
			...
			if ((PA_LetterPos.Letter[i].Letter == ' ') && width) {
				...
				justify += change;
				width -= change; // Moins � corriger par la suite...
				spaces--;
			}
		}
	}
...
void PA_DoLetterPos(s16 basex, s16 basey, s16 maxx, s16 maxy, const char* text, u16 color, u8 size, s32 limit) {
	...
	for (i = 0; (i < limit + 100) && (text[i]); i++) {
		...
		if ((letter == '\n') || ((x + lx > maxx) && (letter != ' '))) { // retour ligne, ou on d�passe...
			...
			if (letter == '\n')PA_AddLetterPos(letter, x, y, size, color); // Retour ligne � mettre dans le tableau
			else if (letter == ' ') PA_AddLetterPos(letter, x, y, size, color); // Si espace, on met � la suite
			else {
				// Sinon, on revient en arri�re pour pas couper un mot !
				while (text[i-1] != ' ') {
			...
			// Retour ligne ! Voir si besoin d'aligne le texte d'un c�t� ou de l'autre, centrer, etc...
			PA_DoAlign(letterstart, x, maxx, (letter != '\n')); // Autorise le justify que si pas '\'n
			x = basex;
			y += pa_bittextpoliceheight[size] + textinfo.linespacing;
			letterstart = PA_LetterPos.NLetters; // Premi�re lettre de cette ligne !
